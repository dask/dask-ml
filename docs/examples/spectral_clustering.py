...
This script compares how the SpectralClustering implementations in
scikit-learn and dask-ml scale with the number of samples. The dask-ml
implementation uses the Nyström Method to approximate the large
``n_samples x n_samples`` affinity matrix that spectral clustering
usually requires. The ``n_components`` parameter controls the tradeoff
