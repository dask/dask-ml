...
def test_pca_randomized_solver():
    ...
    for n_comp in np.arange(1, dX.shape[1]):
        pca = dd.PCA(n_components=n_comp, svd_solver='randomized',
                     random_state=0, iterated_power=4)
...
def test_whitening():
    ...
    for solver, copy in product(solver_list, (True, False)):
        ...
        pca = dd.PCA(n_components=n_components, whiten=True, copy=copy,
                     svd_solver=solver, random_state=0, iterated_power=4)
        # test fit_transform
        ...
        X_ = dX.copy()
        pca = dd.PCA(n_components=n_components, whiten=False, copy=copy,
                     svd_solver=solver, random_state=0).fit(X_)
...
def test_explained_variance():
    ...
    rpca = dd.PCA(n_components=2, svd_solver='randomized',
                  random_state=42, iterated_power=1).fit(dX)
    assert_array_almost_equal(pca.explained_variance_,
    ...
    rpca = dd.PCA(n_components=2, svd_solver='randomized',
                  random_state=rng, iterated_power=2).fit(dX)
    assert_array_almost_equal(pca.explained_variance_ratio_,
...
def test_singular_values():
    ...
    pca = sd.PCA(n_components=2, svd_solver='full',
                 random_state=rng).fit(X)
    apca = dd.PCA(n_components=2, svd_solver='full',
                  random_state=rng).fit(dX)
    rpca = dd.PCA(n_components=2, svd_solver='randomized',
                  random_state=rng, iterated_power=4).fit(dX)
        ...
        for data in [X]:
            ...
            if solver == 'arpack':
                ...
                assert_raises_regex(ValueError,
                                    ...
                                    dd.PCA(n_components, svd_solver=solver)
                                    .fit, data)
...
def test_randomized_pca_check_projection():
    ...
    Yt = dd.PCA(n_components=2, svd_solver='randomized',
                random_state=0).fit(X).transform(dXt)
    Yt /= np.sqrt((Yt ** 2).sum())
...
def test_randomized_pca_check_list():
    ...
    X = [[1.0, 0.0], [0.0, 1.0]]
    X_transformed = dd.PCA(n_components=1, svd_solver='randomized',
...
@pytest.mark.parametrize('svd_solver', [
    'full',
    pytest.mark.xfail(reason="svd_compressed promotes")('randomized'),
])
